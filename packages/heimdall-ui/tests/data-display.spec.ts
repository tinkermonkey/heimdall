...
test.describe('Data Display Components', () => {
  ...
  const setupTokensAndFonts = `
    /* Self-hosted fonts loaded from /fonts directory (WOFF2) */
    @font-face {
      font-family: 'Inter';
  ...
  test('Table interactive behavior - sort cycling', async ({ page }) => {
    ...
    // Verify sorted status is displayed
    const sortedText = page.locator('text=/Sorted by:/')
    await expect(sortedText).toBeVisible()

    // Click again to sort descending
    await firstSortableHeader.click()

    // Verify different sort icon (chevron direction changed)
    await expect(sortIcon).toBeVisible()
  })

  test('Table interactive behavior - row key resolution', async ({ page }) => {
    // Verify rows can be identified by their keys
    const firstRow = page.locator('tbody tr').first()
    const rowKey = await firstRow.getAttribute('key')
  ...
  // Visual regression tests for legacy HTML-based tests
  test('stat-tile variants (visual regression)', async ({ page }) => {
